...
use Omea\Entity\Main\Commandes;
use Omea\Entity\Main\Transaction;
use Omea\Entity\Main\TransactionSap;
...
class OrderManager
{
    ...
    public function create($clientId, $distributorId, $providerId, $articleId, $isReady)
    {
        ...
        $sapNivSubOffre = $this->em->getRepository('Omea\Entity\Main\SapNivSubOffre')->find($forfait->getIdNivSub());
        $transactionSAP->setSapNivSubOffre($sapNivSubOffre);
        $sapHierarchieOffre = $this->em->getRepository('Omea\Entity\Main\SapHierarchieOffre')->find($forfait->getIdHierOffre());
    ...
    {
        $client = $this->em->getRepository('Omea\Entity\Main\Client')->find($clientId);
    ...
    {
        $distributor = $this->em->getRepository('Omea\Entity\Main\Distributeurs')->findOneBy(
            array(
                'idDis' => $distributorId,
                'idMag' => $providerId
            )
    ...
    {
        $stockMsisdn = $this->em->getRepository('Omea\Entity\Main\StockMsisdn')->findOneBy(
            array(
                'idClient' => $clientId
            )
        );
    ...
    {
        $article = $this->em->getRepository('Omea\Entity\Main\Article')->find($articleId);
