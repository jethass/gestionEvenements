...
class EligibilityService extends AbstractService
{
    ...
    {
        $stockNsce = $this->emMain->getRepository('Omea\Entity\Main\StockNsce')->findBy(
            array(
                'msisdn' => $msisdn,
                'etat' => 1,
                'idNetwork' => 1
    ...
    {
        $diseAbo = $this->emMain->getRepository('Omea\Entity\Main\DiseAbonnement')->find($numAbo);
        if (!empty($diseAbo)) {
    ...
    {
        $recou = $this->emRecou->getRepository('Omea\Entity\Recouvrement\RecouClient')->find($idClient);
        if (!empty($recou)) {
            throw new EligibilityException(
    ...
    {
        $resiliation = $this->emMain->getRepository('Omea\Entity\Main\Resiliation')->findBy(
